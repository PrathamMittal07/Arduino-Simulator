...
function Sidebar({
  ...
  // --- STRICT MUTUAL EXCLUSION FILTER ---
  // We convert both values to String() to ensure '2' == 2 matches correctly.
  
  // LED Options: Show all pins EXCEPT the one assigned to Button
  const ledOptions = pinOptions.filter(
    (pin) => String(pin) !== String(pinConfig.button)
  );

  // Button Options: Show all pins EXCEPT the one assigned to LED
  const buttonOptions = pinOptions.filter(
    (pin) => String(pin) !== String(pinConfig.led)
  );
